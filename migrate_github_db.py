...
def transfer_data(table_name, columns):
    ...
    offset = 0
    while True:
        sqlite_cursor.execute(f"SELECT {columns_str} FROM {table_name} ORDER BY id LIMIT {BATCH_SIZE} OFFSET {offset};")
...
tables = {
    "comments": ["id", "node_id", "url", "issue_id", "issue_url", "user", "created_at", "updated_at",
                 "author_association", "body", "reactions"],
    "issues": ["id", "url", "repository_id", "repository_url", "node_id", "number", "title", "owner",
    ...
    "logs": ["id", "last_org_id", "last_user_id", "last_org_repository_id", "last_user_repository_id", "created_at"],
    "organizations": ["id", "login", "node_id", "description"],
    "repositories": ["id", "node_id", "name", "full_name", "private", "owner", "owner_type", "owner_id",
